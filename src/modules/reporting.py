...
class Performance:
    ...
    def _calculate_loss_error_corrs(self, pred_d, loss):
        ...
        for key, data in pred_d.items():
            ...
            d['hist2d'] = [loss_stripped_clipped, abs_data]
            d['xlabel'] = 'Loss value'
            d['ylabel'] = 'Abs(%s)'%(key)
            d['savefig'] = savepath
            d['title'] = title
    ...
    def _calculate_performance(self, energy, pred_dict, crs_dict, true_dict, n_doms):
        ...
        for name, func, model_key in zip(
            ...
            print(get_time(), 'Wilcoxon (alternative: "Greater"): p = %.2f'%(p_val))
            ...
            for key in self._true_keys:
                ...
                if self.meta_pars['group'] in CLASSIFICATION:
                    ...
                    if (
                        ...
                        (key == 'p_numu' and code == 140000) or
                        (key == 'p_nutau' and code == 160000)
                    ):
    ...
    def _get_n_doms(self, event_ids):
        ...
        n_doms = [data[key[0]] for event_id, data in fetched.items()]
    ...
    def _binarize_ids(self, particle, pred, target):
        n = target[particle].shape[0]
        sig_indices = np.nonzero(target[particle])[0]
        r = np.arange(n)
        bg_indices = r[~np.isin(r, sig_indices)]

        return sig_indices, bg_indices
    
    def _make_PIDhist_ROC_ConfMatrix(self, pred_dict, target_dict, energy):
        ...
        # Make ROC-curves and histogram of scores for all binarized predictions
        onenum = 0
        n_targets = len(pred_dict.keys())
        for particle in pred_dict:
            sig_ids, bg_ids = self._binarize_ids(particle, pred_dict, target_dict)
            n_sig = sig_ids.shape[0]
            n_bg = bg_ids.shape[0]

            sig_scores = pred_dict[particle][sig_ids]
            bg_scores = pred_dict[particle][bg_ids]

            # PID HISTOGRAMS
            labels = [
                r'%s, N = %.0f $\cdot 10^3$'%(particle, n_sig/1000.0), 
                r'BG, N = %.0f $\cdot 10^3$'%(n_bg/1000.0), 
                ]
            ...
            d = {'data': [
                ...
                'yscale': 'log',
                'savefig': '/'.join([
                    base_path, 'PID_'+particle+'_dist.png'
                ])
            }
            _ = make_plot(d)

            # ROC CURVE
            sig_fpr, sig_tpr, _ = roc_curve(
                target_dict[particle], pred_dict[particle]
                )
            
            sig_auc = auc(
                sig_fpr, sig_tpr
                )
            auc_err = calc_auc_err(target_dict[particle])
            
            d = {
                'x': [sig_fpr],
                'y': [sig_tpr],
                'title': r'%s ROC curve'%(particle),
                'xlabel': 'False Positive Rate',
                'ylabel': 'True Positive Rate',
                'label': [r'AUC = %.3f $\pm$ %.3f'%(sig_auc, auc_err)],
                'savefig': '/'.join([
                base_path, 'ROC_'+particle+'.png'
                ])
            }
            _ = make_plot(d)
            onenum += sig_auc
            ...
            # Earlier we got: nue_scores, nue_energies, numu_scores, numu_energies
            # all arrays
            indices = np.arange(energy.shape[0])
            E_sorted, indices_sorted = sort_pairs(energy, indices)
            counts, bin_edges = np.histogram(E_sorted, bins=N_BINS_CLASS_PLOTS)
            centers = calc_bin_centers(bin_edges)
            _, indices_binned = bin_data(E_sorted, indices_sorted, bin_edges)
            colors = [mpl.cm.plasma(x) for x in np.linspace(0.0, 0.8, N_BINS_CLASS_PLOTS) ]
            x = []
            y = []
            aucs = []
            entries = []
            labels = []
            for i, ids in enumerate(indices_binned):

                fpr, tpr, _ = roc_curve(
                target_dict[particle][ids], pred_dict[particle][ids]
                )
                bin_auc = auc(fpr, tpr)
                auc_err = calc_auc_err(target_dict[particle][ids])
                x.append(fpr)
                y.append(tpr)
                label = r'AUC %.3f$\pm$%.3f, $\log\,\frac{E}{\mathrm{GeV}}$ %.1f-%.1f'%(
                    bin_auc, auc_err, bin_edges[i], bin_edges[i+1]
                )
                labels.append(label)
            savefig = '/'.join([base_path, 'ROCs_'+particle+'.png'])
            d = {
                'x': x,
                'y': y,
                'xlabel': 'False Positive Rate',
                'ylabel': 'True Positive Rate',
                'xrange': [-0.05, 1.1],
                'title': r'%s vs BG ROC curve vs Energy'%(particle),
                'label': labels,
                'color': colors,
                'savefig': savefig,
            }
        ...
        self.onenumber_performance = onenum/len([key for key in pred_dict])

        # CONFUSION MATRIX
        probs = np.stack(
            [data for particle, data in pred_dict.items()], 
            axis=1
        )
        probs_true = np.stack(
            [data for particle, data in target_dict.items()], 
            axis=1
        )

        preds = np.argmax(probs, axis=1)
        true = np.argmax(probs_true, axis=1)
        labels = [key for key in pred_dict]

        cm = confusion_matrix(true, preds, labels=np.arange(n_targets))
        d = {
            'confusion_matrix': cm,
            'labels': labels,
            'savefig': '/'.join([
